...
def sleep_level():
    if sleep >5:
        print("Niveau d'énergie : " + ("▰"*sleep))
    else:
        print("Niveau d'énergie : " + ("▰"*sleep) + " *il somnole*")
...
with open('bmemory', 'wb') as fichier: #il ouvre le fichier
    memory = pickle.Pickler(fichier) #il appelle la fonction pickle pour dire qu'il va écrire dans le fichier
    # enregistrement

bstats = [weight, cuddle, sleep, hungry]

with open('bmemory', 'wb') as fichier:
    memory = pickle.Pickler(fichier)
    memory.dump(bstats)
